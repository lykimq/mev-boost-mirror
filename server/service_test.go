...
func TestMalformedRelayURLs(t *testing.T) {
	// The actual pubkey length used in the service
	validPubkey := "0xac6e77dfe25ecd6110b8e780608cce0dab71fdd5ebea22a16c0205200f2f8e2e3ad3b71d3499c54ad14d6c21b41a37ae"
	...
	testCases := []struct {
		...
		{
			name: "Valid Flashbots URL",
			relays: []types.RelayEntry{
				{URL: mustParseURL(t, "https://"+validPubkey+"@boost-relay.flashbots.net")},
			},
			wantErr: false,
			explanation: "Standard valid relay URL format",
		},
		{
			name: "No relays configured",
			relays: []types.RelayEntry{},
			wantErr: true,
			explanation: "At least one relay must be configured",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			opts := BoostServiceOpts{
				Log:                   logrus.New().WithField("test", tc.name),
				ListenAddr:            "localhost:12345",
				Relays:               tc.relays,
				GenesisForkVersionHex: "0x00000000",
				RelayCheck:           true,
			...
			if tc.wantErr {
				require.Error(t, err)
				require.Equal(t, errNoRelays, err)
				t.Logf("Expected error received: %v\nExplanation: %s", err, tc.explanation)
			} else {
				require.NoError(t, err)
				t.Logf("Valid configuration accepted. Explanation: %s", tc.explanation)
			}
		})
	}
}
