...
func (m *BoostService) handleGetHeader(w http.ResponseWriter, req *http.Request) {
	vars := mux.Vars(req)
	slot := vars["slot"] // which slot to propose
	parentHashHex := vars["parent_hash"] // previous block hash
	pubkey := vars["pubkey"] // validator's public key
	...
	// Ask all relays for their best bid
	for _, relay := range m.relays {
		wg.Add(1)
		go func(relay types.RelayEntry) {
			defer wg.Done()
			...
			// Security check to ensure the relay is not trying to propose a different block
			if bidInfo.parentHash.String() != parentHashHex {
				...
				// reject if parent hashes are different
				return
			}
