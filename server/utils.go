...
func checkRelaySignature(bid *builderSpec.VersionedSignedBuilderBid, domain phase0.Domain, pubKey phase0.BLSPubKey) (bool, error) {
	// 1. Get the message root hash
	root, err := bid.MessageHashTreeRoot()
	if err != nil {
		return false, err
	}

	// 2. Get the signature from the bid
	sig, err := bid.Signature()
	if err != nil {
		return false, err
	}

	// 3. Create signing data by combining message root and domain
	signingData := phase0.SigningData{ObjectRoot: root, Domain: domain}

	// 4. Get the final message to verify by hashing the signing data
	msg, err := signingData.HashTreeRoot()
	if err != nil {
		return false, err
	}

	// 5. Verify the signature using BLS (Boneh-Lynn-Shacham)
